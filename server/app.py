...
class OpenEnvOpsAdapter:
    """Thin adapter exposing the local environment through the OpenEnv HTTP interface."""

    _shared_env = OpsRiskEnv()
    ...
    def reset(self, task_id: str = "known_signal_easy") -> OpsObservation:
    ...
    async def reset_async(self, task_id: str = "known_signal_easy") -> OpsObservation:
        return self.reset(task_id=task_id)

    def step(self, action: OpsAction) -> OpsObservation:
    ...
    async def step_async(self, action: OpsAction) -> OpsObservation:
