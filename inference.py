"""
Baseline runner for the operations-risk triage submission.
...
SYSTEM_MESSAGE = """
You are acting as an operations-risk triage analyst.

Read the scenario bundle and reply with exactly one JSON object.
Allowed keys:
- classification
- suspect_drug
- severity_assessment
...
def fetch_reset(task_name: str) -> dict:
    response = requests.post(f"{ENV_URL}/reset", json={"task_id": task_name}, timeout=30)
    response.raise_for_status()
    return response.json()


def submit_action(action: OpsAction) -> dict:
    response = requests.post(f"{ENV_URL}/step", json={"action": action.model_dump()}, timeout=30)
    response.raise_for_status()
    return response.json()
...
def prompt_for_case(observation: dict) -> str:
    return (
        "Assess the following operations-risk scenario.\n\n"
        "Return one final structured judgment.\n\n"
        f"{json.dumps(observation, ensure_ascii=True, indent=2)}\n\n"
        "Focus on whether the incident is novel or already documented, the most plausible root-cause asset "
        "or interaction, the right severity band, and the correct operational next step."
    )


def ask_model(llm: OpenAI, observation: dict) -> OpsAction:
    ...
    return OpsAction(**payload)


def compact_action(action: OpsAction) -> str:
    return f"{action.classification}/{action.suspect_drug}" if action.suspect_drug else action.classification
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the operations-risk baseline agent")
    parser.add_argument("--difficulty", choices=["easy", "medium", "hard", "all"], default="all", help="Which task subset to run")
